          ...
          <div>
             ...
             <div class="border border-green-500/30 bg-gray-900/80 p-4 rounded relative overflow-hidden group">
                ...
                <div class="w-full bg-gray-800 h-2 rounded-full overflow-hidden">
                  <div class="bg-green-500 h-full transition-all duration-500" [style.width.%]="getPoolUsagePercentage()"></div>
                </div>
             </div>
          </div>
...
export class SystemHealthModalComponent {
    ...
    getPoolUsagePercentage(): number {
        if (!this.health || !this.health.containers) return 0;
        const total = this.health.containers.poolSize || 1;
        const available = this.health.containers.available || 0;
