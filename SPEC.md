...
The system uses the MEAN stack (MongoDB, Express, Angular, Node.js) to provide a robust platform for designing and running agentic AI experiments.
...
-   **Execution Engine**: Node.js orchestrator that spawns Python subprocesses for executing Tools and Scripts (keeping the core logic close to the AI ML ecosystem).
...
*   **Members**:
    ...
    *   `variableTypes`: Map<String, String> (Type definitions: `string`, `int`, `float`, `bool`, `enum:[A,B]`).
*   **Methods**:
...
*   **Methods**:
    *   `execute(environment, arguments)`: Runs the python script, returns output, may modify environment.
...
*   **Members**:
    ...
    *   `variableWhitelist`: List<String> (Defines which Environment variables this Role can see).
*   **Methods**:
...
3.  **Plan Designer**
    ...
    *   **Roles**: Accordion list.
        *   Role Detail: System Prompt text area, Model dropdown, Tool selector (multi-select), Variable Whitelist (multi-select).
    *   **Workflow**:
...
4.  **Experiment Monitor (The "Scientist" View)**
    ...
    *   **Layout**:
        ...
        *   **Center Panel**: Role Activity (Current prompt, thinking process, tool output).
...
*   **Tool Failures**: If a Python tool script crashes or returns a non-zero exit code, the Experiment Step should catch this.
    *   Configurable policy: `ABORT_EXPERIMENT` or `CONTINUE_WITH_ERROR` (logging the failure).
*   **LLM Failures**: Network timeouts or API errors from Ollama/Providers should trigger a retry mechanism (exponential backoff) before failing the step.
*   **Infinite Loops**: `maxSteps` in ExperimentPlan enforces a hard stop.
*   **Orphaned Processes**: The Execution Engine must ensure all spawned Python subprocesses are killed if the Node.js parent process terminates.
