...
└── plaintext-secret-store.js     # Dev-only implementation
```
...
### [NEW] [plaintext-secret-store.js](file:///home/andrew/Projects/Code/web/scientist-ai/backend/src/services/secrets/plaintext-secret-store.js)

Dev-only implementation with:
- Internal Mongoose schema `{ key: String (unique), value: String, createdAt, updatedAt }`
- Console warning on instantiation (dev safety)
...
| Factory default | Verify factory returns `PlaintextSecretStore` when env unset |
| Factory explicit | Verify factory returns `PlaintextSecretStore` when env is `plaintext` |
